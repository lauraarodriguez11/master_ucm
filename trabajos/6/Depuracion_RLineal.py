...
from FuncionesMineria import (analizar_variables_categoricas, cuentaDistintos, atipicosAmissing, 
                              ...
                              validacion_cruzada_lm, lm_forward, lm_backward, lm_stepwise, 
                              modelEffectSizes, crear_data_modelo)
...
numericas_input = list(datos_input.select_dtypes(include = ['int', 'int32', 'int64','float', 'float32', 'float64']).columns)
...
#No la convierto en categórica por futuras incompatibilidades con el modelo de regresión logística
...
for rep in range(20):
    modelo1 = validacion_cruzada_lm(5, x_train, y_train, modeloForwBIC['Variables']['cont'], modeloForwBIC['Variables']['categ'], modeloForwBIC['Variables']['inter'])
    ...
    modelo4 = validacion_cruzada_lm(5, x_train, y_train, var_3['cont'], var_3['categ'], var_3['inter'])   
...
num_params = [len(modeloForwBIC['Modelo'].params), len(frec_ordenada['Formula'][0].split('+')),
