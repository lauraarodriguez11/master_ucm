...
from FuncionesMineria_Corrected import (graficoVcramer, mosaico_targetbinaria, boxplot_targetbinaria, 
...
with open('datosVinoDep.pickle', 'rb') as f:
    datos = pickle.load(f)
...
for variable in variables:
    ...
    VCramer = pd.concat([VCramer, pd.DataFrame({'Variable': [variable], 'Objetivo': [varObjCont.name], 'Vcramer': [v_cramer]})], ignore_index=True)
    
for variable in variables:
    v_cramer = Vcramer(datos_input[variable], varObjBin)
    VCramer = pd.concat([VCramer, pd.DataFrame({'Variable': [variable], 'Objetivo': [varObjBin.name], 'Vcramer': [v_cramer]})], ignore_index=True)
