...
os.chdir('G:/Mi unidad/Apuntes titulo propio1/MasterDani/Modulo Portela/Dani') 
...
estadisticos = pd.DataFrame({
    ...
    'Desviación Estándar': notas[variables].std(),
    'Varianza': notas[variables].var(),
    'Coeficiente de Variación': (notas[variables].std() / notas[variables].mean()),
    'Datos Perdidos': notas[variables].isna().sum()  # Cuenta los valores NaN por variable.
})
...
correlacion = pd.DataFrame(np.corrcoef(notas_estandarizadas.T, resultados_pca.T), 
